...
void ChessSquare::onPress() {
    ChessBoard * prnt = reinterpret_cast<ChessBoard*>(parentWidget());
    prnt->buttonPressed(index);
}


ChessBoard::ChessBoard(chessCore::colour side, QWidget *parent)
    : QWidget(parent) {
    ...
    for (i=0; i < 8; i++) {
        for (j=0; j < 8; j++) {
            ...
            connect(squares[i][j], &QPushButton::clicked,
                    squares[i][j], &ChessSquare::onPress);
    ...
    for (i=0; i < 8; i++) {
        for (j=0; j < 8; j++) {
            if (((downSide == chessCore::white) &&
                (firstIndex == 8 * (7 - i) + j))
             || ((downSide == chessCore::black) &&
                 (firstIndex == 8 * i + (7 - j)))) {
                squares[i][j]->setDown(true);
            } else if (((downSide == chessCore::white) &&
                        (secondIndex == 8 * (7 - i) + j)) ||
                       ((downSide == chessCore::black) &&
                        (secondIndex == 8 * i + (7 - j)))) {
                squares[i][j]->setDown(true);
...
void ChessBoard::buttonPressed(int idx) {
    ...
    if (firstIndex == -1) {
        ...
        if (firstIndex != secondIndex) {
            PlayerGUI * prnt = reinterpret_cast<PlayerGUI*>(parentWidget());
